...
class Board extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      humanPlay: async (i, j) => {
        if (
          this.state.symbolToPlay === this.state.humanSymbol &&
          ...
          if (this.state.game.any_winner(this.state.cells) === true) {
            ...
            winnerCells["coords"].forEach((cell) => {
              this.state.updateCells(
                cell[0],
                cell[1],
                winnerCells[this.state.humanSymbol]
              );
            });
            await this.setState({
              gameStatus: "you win",
      ...
      computerPlay: async () => {
        let computerMove = this.state.game.select_move(
          this.state.cells,
        ...
        await this.state.updateCells(
          computerMove["i"],
          computerMove["j"],
          this.state.computerSymbol
        );
        await this.setState({
        ...
        if (this.state.game.any_winner(this.state.cells) === true) {
          ...
          winnerCells["coords"].forEach((cell) => {
            this.state.updateCells(
              cell[0],
              cell[1],
              winnerCells[this.state.computerSymbol]
            );
          });
          await this.setState({
                ...
                {row.map((cell, j) => {
                  return (
                    <div
                      ...
                      className={
                        "cell-properties " +
                        (this.state.hoverCells === true ? "hover-cells" : "")
                      }
                      key={nanoid()}
                      onClick={() => this.state.humanPlay(i, j)}
                    >
                      <img
                        data-testid={`img-${i}-${j}`}
                        alt=""
                        src={this.state.symbols[cell]}
                        height="60px"
                        width="60px"
